...
export const $tokenData = $accessToken.map((token) => {
...
export const $role = $tokenData.map<AppRole>((token) => {
  const jwtRoles = token?.realm_access?.roles ?? []
  if (jwtRoles.includes('PartnerAdmin')) {
    return 'agent'
  }
